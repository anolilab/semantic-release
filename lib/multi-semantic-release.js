...
import { castArray, sortBy, template, uniq } from "lodash-es";
import semanticRelease from "semantic-release";

import createInlinePluginCreator from "./createInline-plugin-creator.js";
...
async function multiSemanticRelease(
    ...
    packages.forEach((package_) => {
        ...
        package_.localDeps = uniq(package_.deps.map((d) => packages.find((p) => d === p.name)).filter(Boolean));
...
async function getPackage(path, { cwd, env, globalOptions, inputOptions, stderr, stdout }) {
    ...
    const packageOptions = await getConfig(dir);
    ...
    const { options, plugins } = await getConfigSemantic({ cwd: dir, env, stderr, stdout }, finalOptions);

    // Return package object.
    return { deps, dir, fakeLogger, manifest, name, options, path, plugins };
}
