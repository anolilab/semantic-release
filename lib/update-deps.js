...
export const getNextPreVersion = (package_) => {
    ...
    const isNewPreRelTag = lastPreRelTag && lastPreRelTag !== package_._preRelease;

    return isNewPreRelTag || !lastVersionForCurrentRelease
...
export const resolveNextVersion = (currentVersion, nextVersion, bumpStrategy = "override", prefix = "") => {
    ...
    if (bumpStrategy === "inherit") {
        ...
        const resolvedChunks = currentChunks.map((chunk, index) =>
            (nextChunks[index]
                ? chunk.replace(/\d+/u, nextChunks[index])
                : chunk)
        );
